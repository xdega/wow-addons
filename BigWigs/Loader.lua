...
do
	...
	local myGitHash = "d3260c4" -- The ZIP packager will replace this with the Git hash.
	local releaseString = ""
	--[=[@alpha@
...
do
	local DBMdotRevision = "20230218045247" -- The changing version of the local client, changes with every new zip using the project-date-integer packager replacement.
	local DBMdotDisplayVersion = "10.0.27" -- "N.N.N" for a release and "N.N.N alpha" for the alpha duration.
	local DBMdotReleaseRevision = "20230217000000" -- Hardcoded time, manually changed every release, they use it to track the highest release version, a new DBM release is the only time it will change.
